...
const Order: React.FC = () => {
  ...
  return (
    <>
      <SEO
        title="Commander en Ligne - Restaurant Indien La Porte du Bengale Épinal"
        description="Commandez vos plats indiens préférés en ligne : tandoori, curry, biryani, naan. Livraison rapide à Épinal. Menu complet disponible."
        keywords="commander indien épinal, livraison indien épinal, menu indien épinal"
        url="https://la-porte-du-bengale.netlify.app/order"
      />
                      ...
                      {items.map((item) => (
                        <div key={item.id} className={`bg-bengal-card rounded-xl overflow-hidden border border-orange-900/10 hover:border-bengal-gold/40 transition-all duration-300 group shadow-lg flex flex-col ${item.available ? '' : 'opacity-60'}`}>
                          <div className="h-48 bg-bengal-dark relative overflow-hidden group-hover:opacity-90 transition-opacity">
                            {item.image ? (
                              <img src={item.image} alt={`${item.name} - Plat indien au restaurant La Porte du Bengale Épinal`} loading="lazy" className="w-full h-full object-cover transform group-hover:scale-110 transition-transform duration-500" />
                            ) : (
